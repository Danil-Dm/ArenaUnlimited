...
void ClientTimerActions( gentity_t *ent, int msec ) {
	...
	if(g_mginf.integer==1){ client->ps.ammo[WP_MACHINEGUN] = 999; }
	if(g_sginf.integer==1){ client->ps.ammo[WP_SHOTGUN] = 999; }
	if(g_bfginf.integer==1){ client->ps.ammo[WP_BFG] = 999; }
	if(g_rginf.integer==1){ client->ps.ammo[WP_RAILGUN] = 999; }
	if(g_cginf.integer==1){ client->ps.ammo[WP_CHAINGUN] = 999; }
...
//unlagged - true ping
	...
	if(client->sess.sessionTeam == TEAM_RED){
	client->ps.speed = g_teamred_speed.integer + zombiespeed;
	if(client->pers.playerclass == PCLASS_FEMALE){
	client->ps.speed *= 1.4;
	}
	}
	...
	if ( ent->botskill == 11 ) {
		client->ps.speed *= 3.25;
	}
	if ( ent->botskill == 12 ) {
		client->ps.speed *= 3.50;
	}
	if ( ent->botskill == 13 ) {
		client->ps.speed *= 3.75;
	}
	if ( ent->botskill == 14 ) {
