...
*/
	...
	*/
	...
	if ( ent->r.svFlags & SVF_BOT ) {
		s = va("n\\%s\\t\\%i\\model\\%s\\hmodel\\%s\\g_redteam\\%s\\g_blueteam\\%s\\c1\\%s\\c2\\%s\\hc\\%i\\w\\%i\\l\\%i\\skill\\%s\\weaponpack\\%s\\tt\\%d\\tl\\%d",
...
//unlagged - backward reconciliation #5
	...
	if ( g_delagHitscan.integer ) {
		...
		trap_SendServerCommand( clientNum, "print \"Arena Unlimited By Danil_Dm\n\"" );
