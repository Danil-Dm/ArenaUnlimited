...
void	NextCustomRound_f( void ) {
	mod_zround += 1;
if(g_roundmode.integer == 1){
trap_SendConsoleCommand( EXEC_APPEND, "exec CustomRound.cfg\n");
}
}
void	ScriptCommand_f( void ) {
	char command[256];
	char command2[256];
	trap_Argv( 1, command, sizeof( command ) );
	trap_Argv( 2, command2, sizeof( command2 ) );
trap_SendServerCommand( -1, va("%s %s\n", command, command2));
}

void	IfRound_f( void ) {					
if(g_roundmode.integer == 1){
trap_SendConsoleCommand( EXEC_APPEND, va("exec rounds/Round%i.cfg\n", mod_zround));
trap_SendConsoleCommand( EXEC_APPEND, va("exec rounds/%s/Round%i.cfg\n", g_currentmap.string, mod_zround));
}
}
...
struct
{
  ...
  { "replacetexture", qfalse, Svcmd_ReplaceTex },
  ...
  { "scriptcommand", qfalse, ScriptCommand_f },
  { "ifround", qfalse, IfRound_f },
