...
static mastercontrol_t masterControl[ITEMGROUP_COUNT] = {
	{ ITEMGROUP_WEAPON, "Weapon:", "Weapons", NULL, weaponitems_list, ARRAY_COUNT(weaponitems_list) },
	{ ITEMGROUP_AMMO, "Ammo:", "Ammo", NULL, weaponammo_list, ARRAY_COUNT(weaponammo_list) },
	{ ITEMGROUP_HEALTH, "Health:", "Health", NULL, healthitems_list, ARRAY_COUNT(healthitems_list) },
	{ ITEMGROUP_ARMOUR, "Armour:", "Armour", NULL, armouritems_list, ARRAY_COUNT(armouritems_list) },
	{ ITEMGROUP_POWERUPS, "Powerups:", "Powerups", NULL, powerupitems_list, ARRAY_COUNT(powerupitems_list) },
	{ ITEMGROUP_HOLDABLE, "Holdables:", "Holdables", NULL, holdableitems_list, ARRAY_COUNT(holdableitems_list) }
};


//static int masterControl_Size = sizeof(masterControl)/sizeof(masterControl[0]);
...
static void StartServer_BothItemMenus_SetupItemControls(int y)
{
	...
	for (i = 0; i < ITEMGROUP_COUNT; i++) {
		...
		s_itemcontrols.tabbedText[i].generic.x	     = 180;
...
static void StartServer_BothItemMenus_AddMasterControls(int y)
{
	...
	for (i = 0 ; i < ITEMGROUP_COUNT; i++)
	{
		...
		s_itemcontrols.groupMaster[i].generic.y		= y + dy;
...
static void StartServer_BothItemMenus_MenuInit(qboolean ingame)
{
	...
	if (ingame) {
		...
		s_itemcontrols.ingame.cancel.generic.x		= 0;
		...
		s_itemcontrols.ingame.accept.generic.x		= 640 - 128;
