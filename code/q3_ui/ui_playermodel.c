...
static animControls_t animControl[] = {
	...
	{ NULL, ANIM_WEAPON1, 11.0, -1.3, "GAUNTLET"},
...
static void PlayerModel_BuildList( void )
{
	...
	numdirs = trap_FS_GetFileList(MODELDIR, "/", dirlist, 2048 );
	dirptr  = dirlist;
	for (i=0; i<numdirs && s_playermodel.nummodels < MAX_PLAYERMODELS; i++,dirptr+=dirlen+1)
	{
		...
		defaultskin = NULL;
		numfiles = trap_FS_GetFileList( va(MODELDIR"/%s",dirptr), "tga", filelist, 2048 );
		fileptr  = filelist;
...
void PlayerModel_DrawModelName( int flags)
{
	...
	else
	{
		...
		textScale = 1.0;
		scaleSmall = UI_ProportionalSizeScale( UI_SMALLFONT );
		style = UI_CENTER;
...
static void PlayerModel_DrawLargeSpinControl( void* self )
{
	...
	UI_DrawProportionalString(t->generic.x, t->generic.y, t->itemnames[ t->curvalue ], style, color);
}
...
//	static char	skinname[32];
	...
	s_playermodel.skinright.generic.x			= SKINARROWS_X + 61;
	...
	s_playermodel.modeldown.generic.y			= MODELARROWS_Y + 61;
	...
	s_playermodel.animate.generic.x		= 630 - 128;
	...
	s_playermodel.animsfx.generic.flags			= QMF_PULSEIFFOCUS|QMF_CENTER_JUSTIFY|QMF_NODEFAULTINIT;
	s_playermodel.animsfx.generic.callback		= PlayerModel_MenuEvent;
	s_playermodel.animsfx.generic.ownerdraw 	= PlayerModel_DrawLargeSpinControl;
	s_playermodel.animsfx.generic.id				= ID_ANIMSFX;
	s_playermodel.animsfx.generic.x				= TEAMANDSFX_COLUMN;
	s_playermodel.animsfx.generic.y				= 480 - 64 + 16;
	s_playermodel.animsfx.itemnames				= anim_sfx;
	PlayerModel_InitLargeSpinControl( &s_playermodel.animsfx );

	s_playermodel.teamModel.generic.type			= MTYPE_SPINCONTROL;
	s_playermodel.teamModel.generic.name			= "";
	s_playermodel.teamModel.generic.flags			= QMF_PULSEIFFOCUS|QMF_CENTER_JUSTIFY|QMF_NODEFAULTINIT;
	s_playermodel.teamModel.generic.callback		= PlayerModel_MenuEvent;
	s_playermodel.teamModel.generic.ownerdraw 	= PlayerModel_DrawLargeSpinControl;
	s_playermodel.teamModel.generic.id				= ID_TEAMMODEL;
	s_playermodel.teamModel.generic.x				= TEAMANDSFX_COLUMN;
	s_playermodel.teamModel.generic.y				= 480 - 64 + 16 + SMALLCHAR_HEIGHT + 2;
	s_playermodel.teamModel.itemnames				= model_dmteam;
	PlayerModel_InitLargeSpinControl( &s_playermodel.teamModel);


	s_playermodel.forceHeadMatch.generic.type			= MTYPE_SPINCONTROL;
	s_playermodel.forceHeadMatch.generic.name			= "";
	s_playermodel.forceHeadMatch.generic.flags			= QMF_PULSEIFFOCUS|QMF_CENTER_JUSTIFY|QMF_NODEFAULTINIT;
	s_playermodel.forceHeadMatch.generic.callback		= PlayerModel_MenuEvent;
	s_playermodel.forceHeadMatch.generic.ownerdraw 	= PlayerModel_DrawLargeSpinControl;
	s_playermodel.forceHeadMatch.generic.id				= ID_FORCEHEADMATCH;
	s_playermodel.forceHeadMatch.generic.x				= BODYPART_COLUMN;
	s_playermodel.forceHeadMatch.generic.y				= 480 - 64 + 16 + SMALLCHAR_HEIGHT + 2;
	s_playermodel.forceHeadMatch.itemnames				= model_simplesplit;
	PlayerModel_InitLargeSpinControl( &s_playermodel.forceHeadMatch);

	s_playermodel.modelChange.generic.type			= MTYPE_SPINCONTROL;
	s_playermodel.modelChange.generic.name			= "";
	s_playermodel.modelChange.generic.flags			= QMF_PULSEIFFOCUS|QMF_CENTER_JUSTIFY|QMF_NODEFAULTINIT;
	s_playermodel.modelChange.generic.callback		= PlayerModel_MenuEvent;
	s_playermodel.modelChange.generic.ownerdraw 	= PlayerModel_DrawLargeSpinControl;
	s_playermodel.modelChange.generic.id				= ID_MODELCHANGE;
	s_playermodel.modelChange.generic.x				= BODYPART_COLUMN;
	s_playermodel.modelChange.generic.y				= 480 - 64 + 16;
	s_playermodel.modelChange.itemnames				= model_partchange;
	PlayerModel_InitLargeSpinControl( &s_playermodel.modelChange);
	
	s_playermodel.colorchange.generic.type			= MTYPE_SPINCONTROL;
	s_playermodel.colorchange.generic.name			= "";
	s_playermodel.colorchange.generic.flags			= QMF_PULSEIFFOCUS|QMF_CENTER_JUSTIFY|QMF_NODEFAULTINIT;
	s_playermodel.colorchange.generic.callback		= PlayerModel_MenuEvent;
	s_playermodel.colorchange.generic.ownerdraw 	= PlayerModel_DrawLargeSpinControl;
	s_playermodel.colorchange.generic.id				= ID_COLORSELECT;
	s_playermodel.colorchange.generic.x				= BODYPART_COLUMN;
	s_playermodel.colorchange.generic.y				= 10 + 16;
	s_playermodel.colorchange.itemnames				= model_colorchange;
	PlayerModel_InitLargeSpinControl( &s_playermodel.colorchange);
