...
void UIE_PlayerInfo_AnimateModel( modelAnim_t* m)
{
	...
	if( crashProtected ) {
		UI_DrawProportionalString( b->generic.x, b->generic.y + b->height / 2, "CRASH DETECTED", UI_LEFT, color_white );
		return;
	}
	...
	if( trap_MemoryRemaining() <= LOW_MEMORY ) {
		UI_DrawProportionalString( b->generic.x, b->generic.y + b->height / 2, "LOW MEMORY", UI_LEFT, color_white );
		return;
	}
	...
	if( m->bUnknownModel ) {
		UI_DrawProportionalString( b->generic.x, b->generic.y + b->height / 2, "UNKNOWN MODEL", UI_LEFT, color_white );
		return;
	}

	if( m->bUnknownHeadModel ) {
		UI_DrawProportionalString( b->generic.x, b->generic.y + b->height / 2, "UNKNOWN HEAD", UI_LEFT, color_white );
		return;
	}
	
	if( m->bUnknownLegsModel ) {
		UI_DrawProportionalString( b->generic.x, b->generic.y + b->height / 2, "UNKNOWN LEGS", UI_LEFT, color_white );
		return;
	}
