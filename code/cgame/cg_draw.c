...
void CG_Draw3DModelCopy( float x, float y, float w, float h, qhandle_t model, qhandle_t skin, vec3_t origin, vec3_t angles ) {
	...
	refdef.fov_x = 90;
	refdef.fov_y = 90;
if(oasb_tool.integer == 70){
ent.hModel = trap_R_RegisterModel( "1" );
...
static void CG_DrawStatusBarHead( float x ) {
	...
	CG_DrawHead( 0 - cg_hudcord.integer, 480 - size, size, size,
...
static void CG_DrawStatusBarFlag( float x, int team ) {
	CG_DrawFlagModel( 0 - cg_hudcord.integer, 480 - ICON_SIZE, ICON_SIZE, ICON_SIZE, team, qfalse );
}
//#endif // MISSIONPACK
...
//		{ 0.2, 1.0, 0.2, 1.0 } , { 1.0, 0.2, 0.2, 1.0 }, {0.5, 0.5, 0.5, 1} };
		{ 0.0f, 0.70f, 0.50f, 1.0f },    // normal
		{ 1.0f, 0.70f, 0.50f, 1.0f },     // low health
		{ 0.5f, 0.5f, 0.5f, 1.0f },     // weapon firing
		{ 0.0f, 0.70f, 0.50f, 1.0f } };   // health > 100
	...
	if ( cent->currentState.weapon) {
		...
		CG_Draw3DModel( 640 + cg_hudcord.integer - TEXT_ICON_SPACE - ICON_SIZE, 432 + 20, ICON_SIZE, ICON_SIZE,
		...
		origin[2] = 0;
		angles[YAW] = 90 + 20 * sin( cg.time / 1000.0 );
		CG_Draw3DModelCopy( 640 + cg_hudcord.integer - TEXT_ICON_SPACE  - 160, 1 + 12, 160, 160,
					   trap_R_RegisterModel( oasb_modifiers.string ), 0, origin, angles );
		}
		}
	}
...
//					   cgs.media.armorModel, 0, origin, angles );
	...
	if ( cent->currentState.weapon ) {
		...
		if ( value > -1 ) {
			...
			CG_DrawField (640 + cg_hudcord.integer - CHAR_WIDTH*3 - TEXT_ICON_SPACE - ICON_SIZE, 432 + 20, 3, value);
			trap_R_SetColor( NULL );
			...
			if ( !cg_draw3dIcons.integer && cg_drawIcons.integer ) {
				...
				if ( icon ) {
					CG_DrawPic( 640 + cg_hudcord.integer - TEXT_ICON_SPACE - ICON_SIZE, 432 + 20, ICON_SIZE, ICON_SIZE, icon );
				}
			}
		}
	}
	...
	CG_DrawField ( 74 - cg_hudcord.integer, 432 + 20, 4, value);
	...
	// stretch the health up when taking damage
	CG_DrawField ( 480, 432 + 20, 4, value);
	CG_ColorForHealth( hcolor );
	trap_R_SetColor( hcolor );
	...
	// stretch the health up when taking damage
	CG_DrawField ( 180, 432 + 20, 4, value);
	CG_ColorForHealth( hcolor );
	trap_R_SetColor( hcolor );
	...
	if (value > 0 ) {
		...
		CG_DrawField (74 - cg_hudcord.integer + CHAR_WIDTH, 408 + 15, 3, value);
		trap_R_SetColor( NULL );
...
static float CG_DrawAttacker( float y ) {
	...
	angles[YAW] = 180;
	angles[ROLL] = 0;
	CG_DrawHead( 640 + cg_hudcord.integer - size, y, size, size, clientNum, angles );

	info = CG_ConfigString( CS_PLAYERS + clientNum );
	name = Info_ValueForKey(  info, "n" );
	y += size;
	CG_DrawBigString( 640 + cg_hudcord.integer - ( Q_PrintStrlen( name ) * BIGCHAR_WIDTH), y, name, 0.5 );
...
static float CG_DrawSpeedMeter( float y ) {
	...
	if (cg_drawSpeed.integer == 1) {
		...
		CG_DrawBigString( 635 + cg_hudcord.integer - w, y + 2, s, 1.0F);
...
if(mins == mod_zround + 1){
	mod_zround += 1;
	trap_SendConsoleCommand("customroundend \n");
}
if(!rus.integer){
	s = va( "Round:%i", mod_zround );
}
...
static float CG_DrawPlayerRound( float y ) {
	char		*s;
	int			w;
	int			mins, seconds, tens;
	int			msec;
...
if(mins == mod_yuround + 1){
	mod_yuround += 1;
	trap_SendConsoleCommand("setup1 ui_zplmd\n");
}
trap_Cvar_Set("r_forceSunTime", va("%i", mod_daytime));
trap_Cvar_Set("r_forceSunAngle", va("%i", mod_dayangle));
if(mod_daydefault >= 0){
trap_Cvar_Set("r_forceSunRestart", va("%i", mod_daydefault));	
}
...
static float CG_DrawLMSmode( float y ) {
	...
	w = CG_DrawStrlen( s ) * SMALLCHAR_WIDTH;
	CG_DrawSmallString( 635 + cg_hudcord.integer - w, y + 2, s, 1.0F);
...
static float CG_DrawCTFoneway( float y ) {
	...
	w = CG_DrawStrlen( s ) * SMALLCHAR_WIDTH;
	CG_DrawSmallStringColor( 635 + cg_hudcord.integer - w, y + 2, s, color);
...
*/
	...
	CG_DrawBigStringColor( 635 + cg_hudcord.integer - w, y + 2, s, color);
...
static float CG_DrawPowerups( float y ) {
	...
	x = 640 - 100 + cg_hudcord.integer - ICON_SIZE - CHAR_WIDTH * 2;
	for ( i = 0 ; i < active ; i++ ) {
    ...
    if (item) {
		  ...
		  CG_DrawPic( 640 + cg_hudcord.integer - size, y + ICON_SIZE / 2 - size / 2,
...
static int CG_DrawPickupItem( int y ) {
	...
	if ( value ) {
		...
		if ( fadeColor ) {
			...
			CG_DrawPic( 8 - cg_hudcord.integer, y, ICON_SIZE, ICON_SIZE, cg_items[ value ].icon );
			if(!rus.integer){
			CG_DrawBigString( ICON_SIZE - cg_hudcord.integer + 16, y + (ICON_SIZE/2 - BIGCHAR_HEIGHT/2), bg_itemlist[ value ].pickup_name, fadeColor[0] );
			}
			if(rus.integer){
			CG_DrawBigString( ICON_SIZE - cg_hudcord.integer + 16, y + (ICON_SIZE/2 - BIGCHAR_HEIGHT/2), bg_itemlist[ value ].pickup_nameru, fadeColor[0] );
			}
			trap_R_SetColor( NULL );
		}
	}
...
#define CHATLOC_X 0 - cg_hudcord.integer
...
static void CG_DrawHoldableItem( void ) {
	...
	if ( value ) {
		CG_RegisterItemVisuals( value );
		CG_DrawPic( 0 - cg_hudcord.integer, (0+SCREEN_HEIGHT / 2), ICON_SIZE, ICON_SIZE, cg_items[ value ].icon );
	}
...
static void CG_DrawPersistantPowerup( void ) {
	...
	if ( value ) {
		CG_RegisterItemVisuals( value );
		CG_DrawPic( 0 - cg_hudcord.integer, (0+SCREEN_HEIGHT / 2) - ICON_SIZE, ICON_SIZE, ICON_SIZE, cg_items[ value ].icon );
	}
}
#endif
//#endif // MISSIONPACK
...
static void CG_DrawDisconnect( void ) {
	...
	s = "Connection Error"; // bk 010215 - FIXME
	w = CG_DrawStrlen( s ) * BIGCHAR_WIDTH;
...
static void CG_DrawLagometer( void ) {
	...
	trap_R_SetColor( NULL );
	CG_DrawPic( x + cg_hudcord.integer, y, 48, 48, cgs.media.lagometerShader );

	ax = x + cg_hudcord.integer;
	ay = y;
	aw = 48;
	ah = 48;
...
static void CG_AndroidAIM( void ) {
	...
	if ( !androidaim.integer ) {
		return;
	}
	if ( sensitivity.integer ) {
		return;
	}
		if(cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR){
		return;	
	...
	team = cgs.clientinfo[ cg.androidtarget	].team;
	targetaim = cg.androidtarget;
		if(cg.snap->ps.persistant[PERS_TEAM] == TEAM_RED){
			if(cgs.clientinfo[ cg.androidtarget ].team == TEAM_BLUE){
				trap_SendConsoleCommand("+button0; -button0;\n");
				targetaim = -2;
				cg.androidtarget = -2;
			}
		}
		if(cg.snap->ps.persistant[PERS_TEAM] == TEAM_BLUE){
			if(cgs.clientinfo[ cg.androidtarget ].team == TEAM_RED){
				trap_SendConsoleCommand("+button0; -button0;\n");
				targetaim = -2;
				cg.androidtarget = -2;
			}
		}
		if(cg.snap->ps.persistant[PERS_TEAM] == TEAM_FREE){
			if(targetaim != -2){
				trap_SendConsoleCommand("+button0; -button0;\n");
				targetaim = -2;
				cg.androidtarget = -2;
