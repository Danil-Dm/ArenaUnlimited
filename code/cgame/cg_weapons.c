...
void CG_RegisterWeapon( int weaponNum ) {
	...
	weaponInfo->weaponModel2 = trap_R_RegisterModel( item->world_model[1] );
	weaponInfo->weaponModel3 = trap_R_RegisterModel( item->world_model[2] );
	weaponInfo->weaponModel4 = trap_R_RegisterModel( item->world_model[3] );
	...
	COM_StripExtension(path, path, sizeof(path));
	strcat( path, "_barrel.md3" );
	weaponInfo->barrelModel2 = trap_R_RegisterModel( path );
	
	Q_strncpyz( path, item->world_model[2], MAX_QPATH );
	COM_StripExtension(path, path, sizeof(path));
	strcat( path, "_barrel.md3" );
	weaponInfo->barrelModel3 = trap_R_RegisterModel( path );
	
	Q_strncpyz( path, item->world_model[3], MAX_QPATH );
	COM_StripExtension(path, path, sizeof(path));
	strcat( path, "_barrel.md3" );
	weaponInfo->barrelModel4 = trap_R_RegisterModel( path );
...
if ( ci->plradius == 4 ) {
	...
	if ( cent->currentState.eFlags & EF_TALK ) {
		gun.hModel = trap_R_RegisterModel( "smartphone.md3" );
	}
...
//Danil_Dm MAX_WEAPONS=16 HACK FOR MANY WEAPONS :) -> MAX_WEAPONS=512 YEAH
...
void CG_DrawWeaponBar1(int count, int bits){
	...
	for ( i = 0 ; i < MAX_WEAPONS ; i++ ) {
		...
		switch(i) {
			...
			case WP_FLAMETHROWER: max = 100; break;
			case WP_ANTIMATTER: max = 5; break;
			default: max = 1; break;
		}
